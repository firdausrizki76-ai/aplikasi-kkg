...
function doGet(e) {
  try {
    ...
    // Backend session check removed to allow custom login flow via localStorage
    ...
    return HtmlService.createHtmlOutput('<h1>Terjadi Kesalahan</h1><p>' + err.toString() + '</p>');
  }
}
...
function getEventAktif() {
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      const eventInfo = {
        id: data[i][0] || ('EVT-' + i),
        nama: data[i][1] || 'Event KKG',
        tanggal: data[i][2] || new Date(),
        lokasi: data[i][3] || '-',
        keterangan: data[i][4] || '-',
        status: 'AKTIF'
      };
    ...
    return { error: "Tidak ada data event satupun yang terisi di baris ke-2 ke atas." };
  } catch (e) {
